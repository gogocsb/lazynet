...
Cmd2TypeNameMap InnerMsgCodec::cmd2Type_;
TypeName2CmdMap InnerMsgCodec::type2Cmd_;

void InnerMsgDispatcher::handle(TcpConnPtr con, BaseHead info, Message* msg)
{
...
Message* InnerMsgCodec::decode(Buffer& buf, BaseHead& info)
{
    if(buf.size() < 20)
    {
...
void InnerMsgCodec::onCodec(uint32_t cmd, string type)
{
...
uint32_t InnerMsgCodec::getCmd(string typeName)
{
    return type2Cmd_[typeName];
}

string InnerMsgCodec::getType(uint32_t cmd)
{
...
void InnerMsgCodec::encode(Message* msg, Buffer& buf)
{
...
void InnerMsgCodec::encode(Message* msg, uint32_t err, uint32_t para1, uint32_t para2, Buffer& buf)
{
    BaseHead info;
    const string& typeName = msg->GetDescriptor()->full_name();
